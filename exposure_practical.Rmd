...
If you wanted to explore other transport modes you could look at using the function `route_transportapi_public()` instead of `route_osrm()`. It allows you to add an additional parameter of `mode = "__modehere__` where `__modehere` is one of  'bus', 'train' or 'tube'. I believe there are some functions within the `stplanr` library that also does walking routing, but you'll need to look at that yourselves.
