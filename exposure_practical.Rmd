...
If you wanted to explore other transport modes you could look at using the function `route_transportapi_public()` instead of `route_osrm()`. It allows you to add an additional parameter of `mode = "__modehere__` where `__modehere__` is one of  'bus', 'train' or 'tube'. I believe there are some functions within the `stplanr` library that also do walking routing, but you'll need to look at that yourselves.
...
Check you understand what the above commands have done before continuining.
...
At this point we could make a graph of the exposure during the journey
...
route_points$incar_no2 <- route_points$no2 * 0.77
...
Note the line ` theme(panel.background = element_rect()) ` in the code above. There are various options that you can use here to change the look and feel of your map. You should use the [ggplot2 reference manual](https://ggplot2.tidyverse.org/reference/theme.html) to find more details. We're not expecting you to be an expert at any of this during the module, but some idea would be good.

## Task / homework

I would like you to make and send to me by 5pm tomorrow, a PNG file that is as similar to the below as you can manage. If you want to work in pairs, as a group, or even as a whole class that's fine; this isn't assessed.
