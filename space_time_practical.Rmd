...
In personal monitoring campaigns data often needs to be combined from multiple measurement devices into one consolidated dataset before it can be analysed. In today's practical we are going to combine some data from 'person 81' of the DeMIST study that Shannon Lim is working on at the moment:

* A microaeth (measures black carbon)
* A GPS device (measures location)

Once we have combined them we will make a table summarising the results, a graphs to visualise it, and a short animated GIF showing the persons movement and exposure.
...
black_carbon_data   <- read_csv('data/black_carbon.csv')
...
location_data <- read_csv('data/gps.csv')
...
The `location_data` has the date and time formatted a little differently. Can you figure out how to sort it out? It'll be something like the below. [Submit your answer in this form.](form here).
...
As we can see, it looks like there has been some instrument error during the campaign. A black carbon measurement of -50, and another of of 90 look like errors to me. Can you figure out how to remove from the dataset all data that is under 25 and over 50, and then plot the data again? I suggest you use the `filter` command from the `dplyr` package. An example of how to use it is `starwars <- filter(starwars, hair_color == "none" & eye_color == "black")`. If you manage it, you will end up with a graph that looks like this:

```{r, echo=F,warning=FALSE, message=FALSE}
ggplot(filter(exposure_dataset, black_carbon > -25 & black_carbon < 50), aes(date, black_carbon)) + 
  geom_path() +
  ggtitle('DeMIST participant 81')
```
...
aggregate(data = exposure_dataset, black_carbon ~ hour(date), FUN=mean)
...
But let's make an animated map instead. There's a new package called `gganimate` which makes this quite easy. Note the last few lines.
...
animate(my_animation, fps = 10, duration = 30,
         renderer = gifski_renderer(file = 'my_animation.gif', loop = TRUE))
